...
def newRestaurant():
    ...
    if request.method == 'POST':
        if request.form['name']=='':
            flash('Error Please enter the restaurant name')
            return redirect(url_for('showRestaurants'))
...
def deleteRestaurant(restaurant_id):
    if 'username' not in login_session:
        return redirect('/login')
    restaurantToDelete = session.query(Restaurant).filter_by(id=restaurant_id).first()
...
def newMenuItem(restaurant_id):
    ...
    if restaurant.user_id !=login_session['user_id']:
        flash('You are not allowed to add new item to this restaurant')
        return redirect(url_for('showRestaurants'))
    if request.method == 'POST':
        if request.form['name']=='':
            flash('Error Please enter the item name')
            return redirect(url_for('showRestaurants'))
...
def editMenuItem(restaurant_id, menu_id):
    ...
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).first()
    if restaurant.user_id !=login_session['user_id']:
        flash('You are not allowed to edit item to this restaurant')
        return redirect(url_for('showRestaurants'))
    if request.method == 'POST':
        if request.form['name']:
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if restaurant.user_id !=login_session['user_id']:
        flash('You are not allowed to delete item to this restaurant')
        return redirect(url_for('showRestaurants'))
    if request.method == 'POST':
